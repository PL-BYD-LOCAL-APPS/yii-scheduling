...
class Event extends \CComponent
{
     ...
     * @param \CConsoleApplication $app
     */
    protected function callAfterCallbacks(\CConsoleApplication $app)
    {
     ...
     * @return bool
     */
    public function isDue(\CConsoleApplication $app)
    {
    ...
    public function onOneServer()
    {
        if ($this->_mutex instanceof FileMutex) {
            throw new \CException('You must config mutex in the application component, except the FileMutex.');
        }
     ...
     * @throws
     */
    public function processOutput(\Closure $closure)
    {
        if (is_null($this->_output) || $this->_output == $this->getDefaultOutput()) {
            throw new \CException("Must direct output to a file in order to custom action performs.");
        }

        return $this->then(function (\CConsoleApplication $app) use ($closure) {
            $textBody = file_get_contents($this->_output);
            call_user_func($closure, $textBody, $app);
        });
    }
